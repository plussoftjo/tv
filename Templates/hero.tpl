...
if($tribe == 1){ $tp = 1000; }else{ $tp = 100; }
$rp = 30000;
...
<div id="attributes"><div class="boxes boxesColor gray"><div class="boxes-tl"></div><div class="boxes-tr"></div><div class="boxes-tc"></div><div class="boxes-ml"></div><div class="boxes-mr"></div><div class="boxes-mc"></div><div class="boxes-bl"></div><div class="boxes-br"></div><div class="boxes-bc"></div><div class="boxes-contents">
  ...
  <div class="attribute power tooltip" title="قدرت هجومی||قدرت هجومی قهرمان ترکیبی از قدرت حمله و دفاع او می‌باشد. به هر اندازه که قدرت هجومی قهرمان بیشتر باشد به همان اندازه صدمه کمتری در ماجراجویی‌های خود خواهد دید.<br><font color='#5dcbfb'>قدرت هجومی: <?php echo $hero['power']; ?> از قهرمان</font>">
            ...
            $powerPoints = 0;
            $heroInv = $database->getHeroInventory($session->uid);
            	if($heroInv['body']>0){
                    $item = $database->getHeroItem($heroInv['body']);
                    if($item['type']==88){
                        $powerPoints = 500;
                    }elseif($item['type']==89){
                        $powerPoints = 1000;
                ...
                if($heroInv['leftHand']>0){
                    $item = $database->getHeroItem($heroInv['leftHand']);
                    if($item['type']==76){
                        $powerPoints = 500;
                    }elseif($item['type']==77){
                        $powerPoints = 1000;
                ...
                if($heroInv['rightHand']>0){
                    $item = $database->getHeroItem($heroInv['rightHand']);
                    if($item['type']==16 || $item['type']==19 || $item['type']==22 || $item['type']==25 || $item['type']==28 || $item['type']==31 || $item['type']==34 || $item['type']==37 || $item['type']==40 || $item['type']==43 || $item['type']==46 || $item['type']==49 || $item['type']==52 || $item['type']==55 || $item['type']==58){
                        $powerPoints = 500;
                    }elseif($item['type']==17 || $item['type']==20 || $item['type']==23 || $item['type']==26 || $item['type']==29 || $item['type']==32 || $item['type']==35 || $item['type']==38 || $item['type']==41 || $item['type']==44 || $item['type']==47 || $item['type']==50 || $item['type']==53 || $item['type']==56 || $item['type']==59){
                        $powerPoints = 1000;
                    }elseif($item['type']==18 || $item['type']==21 || $item['type']==24 || $item['type']==27 || $item['type']==30 || $item['type']==33 || $item['type']==36 || $item['type']==39 || $item['type']==42 || $item['type']==45 || $item['type']==48 || $item['type']==51 || $item['type']==54 || $item['type']==57 || $item['type']==60){
                        $powerPoints = 1500;
                    }
                }
                $powerBar = round(((($hero['power']-$powerPoints)-100)/$ppp), 1);
            ?>
        ...
        <a class="setPoint<?php echo $pStyle; ?>" href="?add=power&a=<?php echo $plevel; ?>&c=<?php echo $heroWrefC; ?>"></a>
			...
			<div class="element points"><?php echo intval(($hero['power']-$powerPoints)/$tp)-1; ?></div>
		</div>

		<div class="clear"></div>
  <div class="attribute offBonus tooltip" title="امتیاز هجومی||باعث افزایش قدرت هجومی تمامی لشکریانی خواهد شد که همراه قهرمان می‌باشند.<br><font color='#5dcbfb'>امتیاز هجومی: <?php echo ($hero['offBonus']/1000); ?>%</font>">
			<div class="element attribName">امتیاز هجومی</div>
			<div class="element current power"><span class="value"><?php echo ($hero['offBonus']/1000); ?></span>%</div>
			<div class="element progress">
				<div class="bar-bg">
  ...
  <div class="attribute defBonus tooltip" title="امتیاز دفاعی||باعث افزایش قدرت دفاعی تمامی لشکریانی خواهد شد که همراه قهرمان می‌باشند.<br><font color='#5dcbfb'>امتیاز دفاعی: <?php echo ($hero['defBonus']/1000); ?>%</font>">
			<div class="element attribName">امتیاز دفاعی</div>
			<div class="element current power"><span class="value"><?php echo ($hero['defBonus']/1000); ?></span>%</div>
			<div class="element progress">
				<div class="bar-bg">
