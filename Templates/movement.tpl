<?php
$aantal=(count($database->getMovement2(4,$village->wid,1))+count($database->getMovement2(34,$village->wid,1))+count($database->getMovement2(7,$village->wid,0))+count($database->getMovement2(3,$village->wid,1))+count($database->getMovement2(3,$village->wid,0))+count($database->getMovement2(9,$village->wid,0))+count($database->getMovement2(5,$village->wid,0)));
...
for($i=0;$i<$aantal;$i++){
	if($aantal2[$i]['attack_type'] == 2 || $aantal2[$i]['spy'] > 0)
		$aantal -= 1;
	}
	if($aantal > 0){
...
for($i=0;$i<$aantal;$i++){
	if(($aantal2[$i]['attack_type']==3) or ($aantal2[$i]['sort_type']==9) or ($aantal2[$i]['attack_type']==4)){
		$lala -= 1; }
}
	if($lala > 0){
			foreach($aantal2 as $receive) {
