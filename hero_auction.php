...
if($_GET['action']=='sell' && $_GET['abort']){
	$check = $database->query_return("SELECT * FROM `".TB_PREFIX."auction` WHERE `id` = '".$_GET['abort']."' AND `owner` = '".$session->uid."'");
	if($check['bids']==0 && $check['finish']==0){
...
if($_GET['action']=='sell' && $_POST['a']=='e45'){
	if($query <= 100){
		...
		if($_POST['amount'] > 0 && $_POST['amount'] <= $data['num'] && $session->uid == $data['uid'] && $data){
