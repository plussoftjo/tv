...
public $unread, $nunread = false;
		...
		$this->unread = $this->checkUnread();
			...
			unset($_SESSION['reply']);
				...
				if(isset($post['ft'])) {   
				 ...
				 case "m1":    $this->quoteMessage($post['id']);break;    
				 case "m2":   
				 ...
				 $this->sendMessage($post['an'],$post['be'],$post['message']);
					...
					header("Location: nachrichten.php?t=2");    break;    
					...
					if(isset($post['archive'])) {
						    ...
						    $this->archiveMessage($post);   
							 ...
							 $this->unarchiveMessage($post);    
							  ...
							  case "m6":    $this->createNote($post);    break;    
	...
	global $database;    for($i = 1; $i <= 10; $i++) {    
 ...
 $topic = "بدون موضوع";  
  ...
  if($permission[opt7]==1){ 
   ...
   if ($userally != 0) {  
    ...
    $database->sendMessage($allmembers[id],$session->uid,str_fixtext($topic),str_fixtext($text),0); 
	...
	return @str_replace($bug[1] , '&gt;' , $TextVar);
					 ...
					 $topic = str_fixtext($this->wordCensor($topic));    
					 $text = str_fixtext($this->wordCensor($text));   
					  }    if($topic == "") {
						      $topic = "بدون موضوع";    }        
							  if($user == "0") {  
							  
							   $database->sendMessage($user, $session->uid, str_fixtext($topic), str_fixtext($text), 0);    
