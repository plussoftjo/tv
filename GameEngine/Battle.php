...
class Battle {
	...
	public function procSim($post) {
		...
		if(isset($post['a1_v']) && (isset($post['a2_v1']) || isset($post['a2_v2']) || isset($post['a2_v3']) || isset($post['a2_v4']))) {
				...
				if(isset($post['a1_1'])) {
					...
					if($sum > 0) {
						if($post['palast'] == "") { $post['palast'] = 0; }
						if(isset($post['wall1']) && $post['wall1'] == "") { $post['wall1'] = 0; }
						if(isset($post['wall2']) && $post['wall2'] == "") { $post['wall2'] = 0; }
						if(isset($post['wall3']) && $post['wall3'] == "") { $post['wall3'] = 0; }
						if(isset($post['wall4']) && $post['wall4'] == "") { $post['wall4'] = 0; }
						if(isset($post['wall5']) && $post['wall5'] == "") { $post['wall5'] = 0; }
						$post['tribe'] = $target[0];
	...
	function calculateBattle($Attacker,$Defender,$def_wall,$att_tribe,$def_tribe,$residence,$attpop,$defpop,$type,$def_ab,$att_ab,$tblevel,$stonemason,$walllevel) {
		...
		//////////////
		$isoasis = $database->isVillageOases($Defender['wref']);
		$hinventorydata = $database->getHeroInventory($Attacker['id']);
		$hitemdata = $database->getItemData($hinventorydata['bag']);
		$ghafas_unused = $hitemdata['num']*20;
		if($isoasis && $hinventorydata['bag'] > 0 && $hitemdata['btype']==9 && $ghafas_unused>0 && $Attacker['hero']>0){
			$useghafas = true;
		}else{
			$useghafas = false;
		}
		$defender_goriz = $database->getUserField($database->getVillageField($Defender['id'],"owner"),"goriz",0);
		////////////////
		...
		if($type == 1){
			...
			if($Defender['hero'] && $defhero['goriz']==0){
				$dp += ($defhero['power']/2);
				$cdp += ($defhero['power']/2);
		...
		if($type == 1){
			...
			$result[1] = round($result[1],8); $result[2] = (!$winner)? pow(($rap/($rdp*$moralbonus)),$Mfactor) : 1;
		...
		if ($units['Att_unit']['hero']>0 && !$useghafas){
			$hero_id=$atkhero['uid'];
		...
		$result['defender_hero_die'] = 0;
		if ($units['Def_unit']['hero']>0 && $defhero['goriz']==0){
			$hero_id=$defhero['uid'];
			$hero_health=$defhero['health'];
			$damage_health=round(100*$result[2]);
			if ($hero_health<=$damage_health){
				$result['defender_hero_die'] = 1;
				$database->modifyHero2('dead', 1, $hero_id, 0);
				$database->modifyHero2('health', 0, $hero_id, 0);
		...
		for($i=$start;$i<=$end;$i++) {
			$y = $i-(($att_tribe-1)*10);
			if($defender_goriz==0) {
		 		$max_bounty += ($units['Att_unit'][$i]-$result['casualties_attacker'][$y])*${'u'.$i}['cap'];
			}else{
				$max_bounty += ($units['Att_unit'][$i]*${'u'.$i}['cap']);
			}
		}
