...
class Technology {
	...
	public function getUnitList() {
		...
		$listArray = $holder = array();
		
		
		for($i=1;$i<=50;$i++){
			$holder['u'.$i]['id'] = $i;
			$holder['u'.$i]['name'] = $this->unarray[$i];
			$holder['u'.$i]['amt'] = 0;
		}
		$holder['hero']['id'] = "hero";
		$holder['hero']['name'] = $this->unarray[51];
		if($unitarray['hero'] > 0) {
			$holder['hero']['amt'] = $unitarray['hero'];
        }
		foreach($reinfs as $reinf){
			if($reinf['hero']>0){
				$holder['hero']['amt'] += $reinf['hero'];
				break;
			}
		}
		for($i=1;$i<=50;$i++){
			$holder['u'.$i]['id'] = $i;
			$holder['u'.$i]['name'] = $this->unarray[$i];
			$holder['u'.$i]['amt'] = $unitarray['u'.$i];
		}
		foreach($reinfs as $reinf){
			for($i=1;$i<=50;$i++){
				if($reinf['u'.$i]>0) $holder['u'.$i]['amt'] += $reinf['u'.$i];
			}
		}
