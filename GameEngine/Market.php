<?php 
function StrBug($TextVar)
	{
		$bug = array('' , '-' , '+' , '\'' , '"' , '!' , '%' , '*' , 'e' , 'n' , 'p');
		print @str_replace($bug , '' , $TextVar);
	}
################################################################################# 
##  Filename       Market.php                                                  ## 
##  Developed by:  Z0kO                                                        ## 
...
class Market { 
    ...
    private function loadMarket() { 
        ...
        $this->maxcarry = ($session->tribe == 1)? 500000 : (($session->tribe == 2)? 1000000 : 750000); 
    ...
    private function addOffer($post) { 
        global $database,$village,$session; 
        $wood = StrBug(($post['rid1'] == 1))? StrBug($post['m1']) : 0; 
        $clay = StrBug(($post['rid1'] == 2))? StrBug($post['m1']) : 0; 
        $iron = StrBug(($post['rid1'] == 3))? StrBug($post['m1']) : 0; 
        $crop = StrBug(($post['rid1'] == 4))? StrBug($post['m1']) : 0; 
        $availableWood = $database->getWoodAvailable($village->wid); 
        $availableClay = $database->getClayAvailable($village->wid); 
        $availableIron = $database->getIronAvailable($village->wid); 
        $availableCrop = $database->getCropAvailable($village->wid); 
