...
ob_start();
        ...
        class MYSQL_DB {
        	...
        	*****************************************/
        	...
        	function addVillage($wid, $uid, $username, $capital, $alliance) {
        		...
        		$time = time();
        		$q = "INSERT into " . TB_PREFIX . "vdata (wref, owner, name, capital, alliance, pop, cp, celebration, wood, clay, iron, maxstore, crop, maxcrop, lastupdate, created) values 
        ('$wid', '$uid', '$vname', '$capital', '$alliance', 2, 1, 0, 780, 780, 780, 800, 780, 800, '$time', '$time')";
        	...
        	***************************/
			function getMovement2($type, $village, $mode) {
				...
				if($mode==1){
							$spyy = "and " . TB_PREFIX ."attacks.spy = 0";
						}else{
							$spyy = "";
						}
        		switch($type) {
					...
					case 3:
        				...
        				$q = "SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where " . TB_PREFIX . "movement." . $where . " = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 3 and " . TB_PREFIX . "attacks.attack_type != 2 " . $spyy . " ORDER BY endtime DESC";
						break;
        			case 34:
        				$q = "SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where " . TB_PREFIX . "movement." . $where . " = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 3 and " . TB_PREFIX . "attacks.attack_type != 3 and " . TB_PREFIX . "attacks.spy = 0 and " . TB_PREFIX . "attacks.attack_type != 4 or " . TB_PREFIX . "movement." . $where . " = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX ."movement.sort_type = 4 " . $spyy . " ORDER BY endtime DESC";
        				break;
        			case 5:
        				$q = "SELECT * FROM " . TB_PREFIX . "movement where " . TB_PREFIX . "movement." . $where . " = $village and sort_type = 5 and proc = 0";
        				break;
					case 7:
        				$q = "SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where " . TB_PREFIX . "movement." . $where . " = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 3 and " . TB_PREFIX . "attacks.attack_type = 2 or " . TB_PREFIX . "movement." . $where . " = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX ."movement.sort_type = 3 ORDER BY endtime DESC";
        				break;
					case 9:
        				$q = "SELECT * FROM " . TB_PREFIX . "movement where " . TB_PREFIX . "movement." . $where . " = $village and sort_type = 9 and proc = 0";
        				break;
        	...
        	function trainUnit($vid, $unit, $amt, $pop, $each, $time, $mode) {
        		...
        		if(!$mode) {
        			...
        			if(count($queued) > 0) {
        				$time = $queued[count($queued) - 1]['commence'] + max ($queued[count($queued) - 1]['eachtime'],1) * $queued[count($queued) - 1]['amt'];
        			}
        			$now = time();
					if ($time) $now = $time;
...
case 1:
$tt =  "100000,100000,100000,100000,100000,100000";
break;
case 2:
$tt =  "100000,100000,100000,100000,100000,100000";
break;
...
break;
        			...
        			$q = "INSERT into " . TB_PREFIX . "odata VALUES ('" . $basearray['id'] . "'," . $basearray['oasistype'] . ",0,".$tt."," . time() . ",100,3,'Oasis')";
        			$result = mysql_query($q, $this->connection);
        		}
        	}
			...
			function addCLP($uid, $clp) {
				$alliance = $this->getUserField($uid,'alliance',0);
				if($alliance>0){
					mysql_query("UPDATE ".TB_PREFIX."alidata SET clp = clp + ".$clp." WHERE id = ".$alliance);
				}
			...
			function checkLogin($user){
                $q = "SELECT * FROM `".TB_PREFIX."active` WHERE `username` = '$user'";
				$result = mysql_query($q, $this->connection);
			...
			function removeOases($wref) {
                $q = "UPDATE ".TB_PREFIX."odata SET conqured = 0, owner = 3, name = 'Oasis' WHERE wref = $wref";
                return mysql_query($q, $this->connection);
            }
			...
			function addAuction($owner, $itemid, $btype, $type, $amount) {
				...
				if($btype==7 || $btype==8 || $btype==9 || $btype==10 || $btype==11 || $btype==14){
					$silver = $amount;
