...
class Units {
	...
	public function sendTroops($post) {
		global $form, $database, $village, $generator, $session;

		$data = $database->getA2b($post['ckey'], $post['timestamp']);
	...
	private function sendTroopsBack($post) {
		...
		if(($enforce['from']==$village->wid) || ($enforce['vref']==$village->wid)){
			if($database->getVillage($enforce['from'])){
				$to = $database->getVillage($enforce['from']);
			}else{
				$to = $database->getOasisV($enforce['from']);
			}
			$Gtribe = $database->getUserField($to['owner'],'tribe',0);
			$start = ($Gtribe-1)*10+1;
			$end = ($Gtribe*10);
			$u = (($Gtribe-1)*10);
			for($i=$start;$i<=$end;$i++) {
			...
			if($form->returnErrors() > 0) {
				...
				if(true){
					for($i=$start;$i<=$end;$i++){
						$y = $i-$u;
						$database->modifyEnforce($post['ckey'],$i,$post['t'.$y.''],0);
					}
					//get cord 
					...
					$reference = $database->addAttack($enforce['from'],$post['t1'],$post['t2'],$post['t3'],$post['t4'],$post['t5'],$post['t6'],$post['t7'],$post['t8'],$post['t9'],$post['t10'],$post['t11'],3,0,0,0);
